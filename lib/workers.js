        ...
        _logs.compress(logId, newFieldId, (err) => {
          if (!err) {
            _logs.truncate(logId, (err) => {
              if (!err) {
                console.log('Success Truncate log Files');
              } else {
                console.log('Error Truncate log Files', err);
              }
            });
          } else {
            console.log('Error: Compressing log file failed', err);
          }
        });
      });
    } else {
      console.log('Error : could not find any log files');
    }
  });
};
